...
async function deleteCalification(req = request, res = response){
    ...
    try {
        ...
        await promisePool.query("DELETE FROM califications WHERE id_student = ? AND id_subject = ? AND id_school = ? AND id_group = ?", [id_student, id_subject, id_school, id_group]);
        return res.status(200).json({
            ok: true,
            status: 200,
...
async function getCalificationByStudent (req = request, res = response){
    const {id_student, id_subject, id_school, id_group} = req.params;
    try {
        const [student] = await promisePool.query("SELECT id FROM students WHERE id = ?", [id_student]);
        if(!student.length){
        ...
        const [calification] = await promisePool.query("SELECT * FROM califications WHERE id_student = ? AND id_subject = ? AND id_school = ? AND id_group = ?", [id_student, id_subject, id_school, id_group]);
        if(!calification.length){
            return res.status(400).json({
                ok: false,
                status: 400,
